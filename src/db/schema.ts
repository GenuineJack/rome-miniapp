...
/**
 * Monthly happenings — three AI-generated "noteworthy things" per month, surfaced
 * in the Today tab's Happening in Boston section. Generated by a Vercel cron on
 * the 1st of each month and reviewed/overrideable by admin.
 */
export const monthlyHappenings = pgTable("monthly_happenings", {
  id: text("id").primaryKey(),
  month: text("month").notNull(),         // "YYYY-MM"
  slot: integer("slot").notNull(),        // 1, 2, or 3
  title: text("title").notNull(),
  emoji: text("emoji").notNull().default("📅"),
  summary: text("summary").notNull(),     // 1-2 sentence card preview
  body: text("body").notNull(),           // Markdown body for mini-page
  imageUrl: text("image_url"),
  sourceLinks: text("source_links"),      // JSON: { label: string; url: string }[]
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
